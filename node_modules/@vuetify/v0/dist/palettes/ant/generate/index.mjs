...
//#region src/palettes/ant/generate/index.ts
const HEX_RE = /^#(?:[0-9a-f]{3}|[0-9a-f]{6}|[0-9a-f]{8})$/i;
function toRecord(colors) {
	const record = {};
	for (const [index, color] of colors.entries()) record[index + 1] = color;
	return record;
}
function contrast(hex) {
	...
	return (.299 * r + .587 * g + .114 * b) / 255 > .5 ? "#000000" : "#ffffff";
}
/* #__NO_SIDE_EFFECTS__ */
function ant(seed, options = {}) {
	if (!HEX_RE.test(seed)) throw new V0Error(`[@vuetify/v0] Invalid seed color: "${seed}". Expected a hex string (e.g., "#1677ff").`, {
		code: "V0_PALETTE_INVALID_SEED",
		palette: "ant",
		seed
	});
	const { background = "#141414" } = options;
	const light = generate(seed);
	const dark = generate(seed, {
		theme: "dark",
		backgroundColor: background
	});
	const grey = generate("#8c8c8c");
	const greyDark = generate("#8c8c8c", {
		theme: "dark",
		backgroundColor: background
	});
	return {
